...
class InverseKinematicsNode(object):
    def __init__(self):
        ...
        self.sub_car_cmd = rospy.Subscriber("~?????", Twist2DStamped, self.?????????)
        self.pub_wheels_cmd = rospy.Publisher("~?????", WheelsCmdStamped, queue_size=1)
    ...
    def car_cmd_callback(self, msg_car_cmd):
        ...
        # Try to complete the inverse_kinematics
        # Calculate the right motor rotation and left motor rotation
        omega_r = ???????????????????
        omega_l = ???????????????????
